...
* セキュリティ: CSVダウンロードの数式インジェクション（=、+、-、@ で始まる値）を無害化
* セキュリティ: ショートコード掲載ページ一覧の公開表示を公開済み投稿のみに限定（未公開投稿の情報漏洩を防止）
* セキュリティ: キャッシュクリアのリダイレクトを wp_safe_redirect に変更（オープンリダイレクト対策）
* セキュリティ: クライアントIP取得を集約し、未設定時の防御とサニタイズを追加
* 修正: 集計データファイルの保存先をアップロードディレクトリに変更（プラグイン更新時のデータ消失を防止／既存データは自動移行）
* 修正: リバースプロキシ／CDN配下で最初の1人しか投票できない問題に対応（信頼プロキシ設定を追加）
* 修正: 投票日時・集計時点の表示をタイムゾーンに沿って統一（保存・表示ともに標準方式へ）
* 修正: 調査期間が設定されている場合に公開日メタが欠落する不具合を修正
* 修正: フォーマット別データセット一覧のページ数が実際の表示件数とずれる問題を修正
* 修正: サブディレクトリ設置環境でデータセットページのルーティングが正しく動作しない問題を修正
* 改善: 集計データフォーマットへのリンクをサイトのURL設定基準で生成
* 改善: 質問文を管理画面で設定した見出しレベルで出力し、投票フォームの並び順をアクセシビリティに配慮
* 改善: データセット詳細ページの構造化データ（Dataset）の二重出力を解消
* 改善: 未使用コードの削除、メタボックスの説明文を実挙動に合わせて修正
...
* 修正: 初期データなし時のプレビュー表示問題を修正

= 1.0.2 - 2026-01-02 =
* 追加: 調査期間表示機能（最初の投票日〜最後の投票日）
* 追加: 凡例テキスト省略機能（15文字以上で...表示、ホバーで全文表示）
* 追加: データセット出力に調査期間フィールドを追加（CSV/XML/YAML/JSON）
* 追加: 構造化データにtemporalCoverageを追加

= 1.0.1 - 2025-12-05 =
* 修正: データセット詳細ページの構造化データでurl/identifierがfalseになる問題を修正
...
= 1.0.2 =
調査期間表示機能と凡例テキスト省略機能を追加。データセットの情報がより充実しました。

= 1.0.1 =
構造化データの修正。データセット詳細ページのurl/identifierが正しく出力されるようになりました。
